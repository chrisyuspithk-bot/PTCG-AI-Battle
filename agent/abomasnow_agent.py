"""Mega Abomasnow ex official sample - official Kaggle sample (never-crash wrapper).

Deck path: env ABOMASNOW_DECK or deck.csv in cwd.
...
def _resolve_deck_path() -> str:
    env = os.environ.get("ABOMASNOW_DECK")
    if env and os.path.exists(env):
        return env
    if os.path.exists("deck.csv"):
        return "deck.csv"
    here = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(here, "..", "agent_decks", "real_mega_abomasnow_ex.csv")

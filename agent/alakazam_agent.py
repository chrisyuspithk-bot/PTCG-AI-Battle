"""Alakazam best5 (ryotasueyoshi) — community rule pilot (never-crash wrapper).
...
try:
    from agent.alakazam_bench_guard import apply_bench_guard
except ImportError:
    from alakazam_bench_guard import apply_bench_guard


def agent(obs_dict: dict) -> list[int]:
    try:
        raw = _agent_impl(obs_dict)
        bench_on = os.environ.get("ALAKAZAM_BENCH_GUARD", "0") != "0"

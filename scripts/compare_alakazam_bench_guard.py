"""A/B compare Alakazam best5 rules vs +R7 empty-bench guard.
...
def main() -> int:
    ...
    hero_deck = load_deck(DEFAULT_ALAKAZAM_DECK)
    brain_off = make_alakazam_brain(bench_guard=False)
    brain_on = make_alakazam_brain(bench_guard=True)
    ...
    lines = [
        ...
        f"- Suite: `{args.suite}`",
        f"- Hero deck: `{DEFAULT_ALAKAZAM_DECK}`",
        f"- Baseline to beat (no guard, S50): **62.0%** @ n=30",
        "",
    ...
    print(f"Alakazam A/B — {args.games} games/opp — suite={args.suite}\n")
    ...
    beat_baseline = overall_on >= 62.0
    lines.extend(
        [
            "",
            "## Overall",
            "",
            f"- No guard: **{overall_off:.1f}%** (n={total_off}), no_active: **{na_off}**",
            f"- + guard: **{overall_on:.1f}%** (n={total_on}), no_active: **{na_on}**",
            f"- Δ: **{overall_on - overall_off:+.1f} pp** vs paired A/B",
            f"- vs 62% S50 baseline: {'**beats**' if beat_baseline else 'below'} ({overall_on:.1f}%)",
            "",
    ...
    print(f"\nOVERALL  off={overall_off:.1f}%  on={overall_on:.1f}%  "
          f"no_active {na_off}→{na_on}  vs62%={'OK' if beat_baseline else 'NO'}")

    if args.report:
        out = ROOT / "eval" / "alakazam_bench_guard_ab.md"
        out.write_text("\n".join(lines) + "\n", encoding="utf-8")
        print(f"\nReport: {out}")
    return 0

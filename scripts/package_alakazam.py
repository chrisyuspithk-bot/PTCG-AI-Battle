"""Package Alakazam best5 (ryotasueyoshi) into a Kaggle submission tarball.

Produces:
  dist/candidates/ryotasueyoshi_alakazam_best5.tar.gz
  dist/candidates/ryotasueyoshi_alakazam_best5.manifest.json

Dry-run: exec main without __file__ + deck-select -> 60 cards.
Actual upload requires explicit user confirmation (5/day).
"""
...
AGENT_SRC = os.path.join(ROOT, "agent", "alakazam_agent.py")
BENCH_GUARD_SRC = os.path.join(ROOT, "agent", "alakazam_bench_guard.py")
EMPTY_GUARD_SRC = os.path.join(ROOT, "agent", "empty_bench_guard.py")
DECK_SRC = os.path.join(ROOT, "agent_decks", "ryotasueyoshi_alakazam_best5.csv")
NAME = "ryotasueyoshi_alakazam_best5_dragapult_lever"
...
MAIN_PY = '''"""Kaggle cabt submission entry point — Alakazam best5 (standalone agent)."""
import os
import sys
...
from alakazam_agent import agent  # noqa: E402,F401
'''
...
def build() -> None:
    ...
    with open(os.path.join(BUILD_DIR, "main.py"), "w", encoding="utf-8") as f:
        f.write(MAIN_PY)
    shutil.copy2(AGENT_SRC, os.path.join(BUILD_DIR, "alakazam_agent.py"))
    shutil.copy2(BENCH_GUARD_SRC, os.path.join(BUILD_DIR, "alakazam_bench_guard.py"))
    ...
    manifest = {
        "name": NAME,
        "agent": "agent/alakazam_agent.py (best5 + dragapult matchup levers; bench guard optional)",
        "deck": "agent_decks/ryotasueyoshi_alakazam_best5.csv",
        "ladder_benchmark_mu": 659.0,
        "ladder_benchmark_ref": "53913404",

"""Local gate for Abomasnow ex agent vs native field opponents.

  python scripts/gate_abomasnow.py --games 30 --suite full --report
"""
...
def main() -> int:
    ...
    result = gate_abomasnow_matchups(
        suite=args.suite,
    ...
    if args.report:
        path = write_gate_report(result, stem="gate_abomasnow", title="Abomasnow agent gate")
        print(f"\nReport: {path}")
    return 0
